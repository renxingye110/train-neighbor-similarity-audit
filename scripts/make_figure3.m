...
constrainedTop = readtable(fullfile(dataDir, 'figure3_constrained_top20.csv'), 'TextType', 'string');
...
constrainedOnly = scatterData(scatterData.membership == "constrained_only", :);
...
hAud = scatter(ax1, constrainedOnly.max_train_jaccard, constrainedOnly.predicted_score, 52, ...
...
text(ax1, 0.49, 0.11, 'Jaccard threshold', ...
...
lgd1 = legend(ax1, [hAll hShared hRaw hAud], ...
    {'All sequences', 'Shared', 'Raw-only', 'Constrained-only'}, ...
...
for classIndex = 1:numel(classes)
    ...
    counts(2, classIndex) = str2double(composition.count(composition.list_name == "Constrained top 20" & composition.similarity_class == classes(classIndex)));
end
...
set(ax2, 'XTick', [1 2], 'XTickLabel', {'Raw', 'Constrained'});
